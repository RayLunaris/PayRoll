...
const taxSchema = z.object({
  bracketFrom: decimalOrString.optional(),
  bracketTo: z
  ...
  rate: decimalOrString.optional(),
  fixedAmount: decimalOrString.optional(),
  isActive: z.boolean().optional(),
});

export async function taxConfigRoutes(app: FastifyInstance) {
  ...
  app.get('/', {
    ...
    try {
      const data = await db.select().from(taxConfig).orderBy(asc(taxConfig.bracketFrom));
  ...
  app.post('/', {
    ...
    try {
      ...
      const created = await db.insert(taxConfig).values({
        ...body,
        bracketFrom: body.bracketFrom || '0',
        rate: body.rate || '0',
  ...
  app.put('/:id', {
    ...
    try {
      ...
      const body = taxSchema.parse(request.body);
      const updated = await db.update(taxConfig).set(body).where(eq(taxConfig.id, id)).returning();
  ...
  app.delete('/:id', {
    ...
    try {
      const { id } = request.params as { id: string };
      const deleted = await db.delete(taxConfig).where(eq(taxConfig.id, id)).returning();
